...
// zhou: genericController implemented cache and controller framework.
type genericController struct {
	name string
	// zhou: event queue used by each controller
	queue  workqueue.RateLimitingInterface
	logger logrus.FieldLogger
	// zhou: used to handle event queue.
	syncHandler func(key string) error
	// zhou: timer based thread to do anything.
	//       In controller-runtime, we implement in SetupWithManager(), to perform timer processing.
	resyncFunc   func()
	resyncPeriod time.Duration
	// zhou: extra waiting for cache sync
...
// zhou: work as "controller-runtime/pkg/internal/controller/controller.go", handle event queue.
...
func (c *genericController) Run(ctx context.Context, numWorkers int) error {
	...
	// zhou: multi-threads for syncHandler
	if c.syncHandler != nil {
		wg.Add(numWorkers)
		for i := 0; i < numWorkers; i++ {
			go func() {
				// zhou: running "c.runWorker" every "time.Second" until stop channel is closed.
				//       But "c.runWorker" will not return. So, "time.Second" will not take effect.
	...
	// zhou: dedicated thread for resyncFunc, NOT sharing with Reconcile()
	if c.resyncFunc != nil {
		if c.resyncPeriod == 0 {
			// Programmer error
		...
		wg.Add(1)
		go func() {
			// zhou: c.resyncFunc() should not block. So it will run every "c.resyncPeriod" == "time.Minute".
...
// zhou: block to run controller's "Reconcile()"
...
func (c *genericController) processNextWorkItem() bool {
	...
	// zhou: invoke user's callback function.
